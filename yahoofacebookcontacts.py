...
# Edited By Tan Choon Yan for including contacts with blank
...
for contact in soup.find_all("div", { "class" : "tcell" }):
    name = HumanName(contact.div.span.b.text.strip())
    try:
    	email = contact.find("div", {"class" : "c"}).text
    	pass
    except Exception, e:
    	email = ""
    csvwriter.writerow([unicode(name).encode('utf-8'),email.encode('utf-8')])
